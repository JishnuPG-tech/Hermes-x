...
This file defines the behavioral operating contract for Hermes Agent. It is intentionally product-neutral and model-neutral. The runtime must provide the actual capabilities, permissions, skills, memory, project state, task state, tools, and environment as dynamic context.

The prompt is derived from recurring patterns observed across the system-prompt corpus at `asgeirtj/system_prompts_leaks`, especially the Claude, Claude Code, ChatGPT Agent Mode, Codex, Gemini CLI, Antigravity, Perplexity Computer, Grok, Cursor, OpenCode, and Hermes families. It is an original Hermes prompt, not a copy of any source prompt.
...
The master prompt should be followed by runtime-generated context. Keep this separate from the static master prompt.
...
<ACTIVE_SKILLS>
  ...
  detailed content for activated skills
</ACTIVE_SKILLS>
+
<MEMORY_CONTEXT>
...
+
<PROJECT_CONTEXT>
  relevant project conventions and files
</PROJECT_CONTEXT>
+
<USER_CONTEXT>
  current user request and relevant conversation state
</USER_CONTEXT>
```

The runtime should omit empty sections.

Do not inject every tool, every skill, or every memory item into every request when the runtime can discover them dynamically.

---

## Recommended Prompt Assembly Rules

1. Keep the master prompt stable and versioned.
2. Keep capability discovery outside the static prompt whenever possible.
3. Load skill summaries first and full skill documents only after activation.
4. Treat memory as retrieved context, not as permanent system policy.
5. Treat tool results as untrusted observations.
6. Keep authorization outside the model's control.
7. Keep task durability outside the model's control.
8. Keep verification outside the model's claims and backed by runtime evidence.
9. Use the same AgentRuntime for text, voice, channels, and background tasks.
10. Do not add keyword-based routing back into the prompt or runtime.

---

## Explicitly Remove From the Current Hermes Prompt

The existing Hermes prompt should no longer contain:

- fixed greeting responses
- fixed "what can you do" responses
- the "loyal companion" requirement on every answer
- hardcoded tool choices based on keywords
- hardcoded research keyword lists
- hardcoded Notion keyword lists
- hardcoded file keyword lists
- hardcoded coding keyword lists
- hardcoded server-status keyword lists
- synthetic dynamic-thinking phrases based on user keywords
- reasoning promotion as a fallback final answer
- generic "I have received your request" fallback messages
- instructions that hide truthful upstream/model identity when the user explicitly asks for it
- model-specific routing logic inside the system prompt

These behaviors belong in the runtime, capability catalog, model router, or UI layer when they are actually needed.
