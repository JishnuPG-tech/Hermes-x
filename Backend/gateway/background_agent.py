...
async def execute_task_iteration(task_id: str):
    ...
    try:
        if task_type == "bash":
            ...
            # Autonomous LLM Agent Execution with canonical AgentRuntime
            ...
            res = await runtime.execute_turn(
                objective=f"[24x7 Scheduled Autonomous Task]: {instruction}",
                context=context,
                model=task.get("model"),
            )
            result_summary = res.get("response", "").strip() or (
                f"Execution failed: {', '.join(res.get('errors', []))}"
                if res.get("errors")
