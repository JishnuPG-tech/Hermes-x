...
interface SessionDao {
    @Query("SELECT * FROM sessions WHERE user_id = :userId ORDER BY updated_at DESC")
    fun getSessions(userId: String): Flow<List<SessionEntity>>

    @Query("SELECT * FROM sessions WHERE user_id = :userId ORDER BY updated_at DESC")
    suspend fun getSessionsList(userId: String): List<SessionEntity>

    @Query("SELECT * FROM sessions WHERE id = :sessionId AND user_id = :userId LIMIT 1")
    ...
    @Query("UPDATE sessions SET title = :title, updated_at = :updatedAt WHERE id = :sessionId AND user_id = :userId")
    suspend fun updateSessionTitle(sessionId: String, userId: String, title: String, updatedAt: Long = System.currentTimeMillis())

    @Query("DELETE FROM sessions WHERE id = :sessionId AND user_id = :userId")
...
interface MessageDao {
    @Query("SELECT * FROM messages WHERE session_id = :sessionId AND user_id = :userId ORDER BY timestamp ASC")
    ...
    @Query("DELETE FROM messages WHERE session_id = :sessionId AND user_id = :userId")
